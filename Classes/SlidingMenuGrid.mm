...
-(CCMenuItem*) GetItemWithinTouch:(UITouch*)touch
{
	...
	//int num_items = [self children].count;
	//int i = 0;
	for (CCMenuItem* item in [self children])
	{
...
-(BOOL) ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    NSLog(@"touchOrigin X: %f || Y:%f",touchOrigin.x, touchOrigin.y);
    if ((touchOrigin.x < 60.0f)&&(touchOrigin.y < 60.0f)) {
        return NO;
    }
